...
const AuthScreen = () => {
  ...
  return (
    <div style={{ minHeight:"100vh",background:`linear-gradient(135deg,${C.navy} 0%,#2d4e7e 100%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}>
      <div style={{ background:C.surface,borderRadius:20,padding:"40px 32px",width:"100%",maxWidth:380,boxShadow:"0 20px 60px rgba(0,0,0,0.3)",textAlign:"center" }}>
        <div style={{ fontSize:56,marginBottom:16 }}>🍇</div>
        <h1 style={{ fontSize:26,fontWeight:800,color:C.navy,marginBottom:6 }}>Fruttino CRM</h1>
        <p style={{ fontSize:14,color:C.soft,marginBottom:32 }}>Panel zarządzania sprzedażą</p>
        <button onClick={login} disabled={loading} style={{ width:"100%",padding:"14px 20px",borderRadius:12,border:`1px solid ${C.border}`,background:C.surface,cursor:loading?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:12,fontSize:15,fontWeight:600,color:C.text,fontFamily:"inherit",boxShadow:"0 2px 8px rgba(0,0,0,0.08)",opacity:loading?0.7:1 }}>
        ...
        <p style={{ fontSize:11,color:C.soft,marginTop:24,lineHeight:1.6 }}>Dostęp tylko dla autoryzowanych użytkowników.</p>
      </div>
    </div>
  );
};

const TopBar = ({ tab, setTab, stats, user, onLogout }) => {
  const tabs = [
    ...
    <div style={{ background:C.navy,position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 8px rgba(0,0,0,0.2)" }}>
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px 10px" }}>
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
          <div style={{ width:32,height:32,borderRadius:8,background:"rgba(255,255,255,0.13)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16 }}>🍇</div>
          <div>
            <div style={{ fontSize:14,fontWeight:700,color:"#fff",lineHeight:1 }}>Fruttino CRM</div>
            <div style={{ fontSize:10,color:"rgba(255,255,255,0.4)",letterSpacing:1,marginTop:1 }}>PANEL GŁÓWNY</div>
          </div>
        </div>
        <div style={{ display:"flex",alignItems:"center",gap:8 }}>
          <div style={{ width:30,height:30,borderRadius:"50%",overflow:"hidden",background:"rgba(255,255,255,0.15)",display:"flex",alignItems:"center",justifyContent:"center" }}>
            {user?.user_metadata?.avatar_url
              ? <img src={user.user_metadata.avatar_url} style={{ width:30,height:30,objectFit:"cover" }}/>
              : <span style={{ fontSize:11,fontWeight:700,color:"#fff" }}>{user?.email?.slice(0,2).toUpperCase()}</span>
            }
          </div>
          <button onClick={onLogout} style={{ background:"rgba(255,255,255,0.1)",border:"none",borderRadius:8,padding:"5px 10px",fontSize:11,color:"rgba(255,255,255,0.7)",cursor:"pointer",fontFamily:"inherit" }}>Wyloguj</button>
        </div>
      </div>
      <div style={{ display:"flex",borderTop:"1px solid rgba(255,255,255,0.08)" }}>
        {tabs.map(t => (
          <button key={t.id} onClick={()=>setTab(t.id)} style={{ flex:1,padding:"9px 4px",background:"transparent",border:"none",borderBottom:tab===t.id?"2px solid #60a5fa":"2px solid transparent",color:tab===t.id?"#fff":"rgba(255,255,255,0.45)",fontSize:11,fontWeight:tab===t.id?600:400,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:3 }}>
            <span style={{ fontSize:13 }}>{t.icon}</span>
            <span>{t.label}</span>
            {t.badge>0 && <span style={{ background:"#ef4444",color:"#fff",borderRadius:100,fontSize:9,fontWeight:700,padding:"1px 5px" }}>{t.badge}</span>}
          </button>
        ))}
      </div>
    </div>
...
const StatRow = ({ stats }) => (
  <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,padding:"12px 12px 0" }}>
    {[{label:"Nowe",value:stats.newOrders,color:C.amber},{label:"Produkty",value:stats.totalProducts,color:C.blue},{label:"Kanały",value:6,color:C.green},{label:"Niski stan",value:stats.lowStock,color:C.red}].map(s=>(
      <div key={s.label} style={{ background:C.surface,borderRadius:10,border:`1px solid ${C.border}`,padding:"11px 8px",borderTop:`3px solid ${s.color}`,textAlign:"center" }}>
  ...
  return (
    <div style={{ padding:"12px 12px 20px" }}>
      <div style={{ display:"flex",gap:8,marginBottom:12,flexWrap:"wrap" }}>
        ...
        <button onClick={load} style={{ padding:"7px 14px",borderRadius:8,border:`1px solid ${C.border}`,background:C.surface,fontSize:12,cursor:"pointer",color:C.mid,fontFamily:"inherit" }}>↻</button>
      </div>
      ...
      {showForm&&(
        <Card style={{ padding:"16px",marginBottom:12,borderLeft:`4px solid ${C.accent}` }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8 }}>
            <div><div style={{ fontSize:11,color:C.soft,marginBottom:4 }}>SKU *</div><input {...inp} placeholder="MOR-500" value={form.sku} onChange={e=>setForm({...form,sku:e.target.value})}/></div>
            <div><div style={{ fontSize:11,color:C.soft,marginBottom:4 }}>Cena PLN *</div><input {...inp} type="number" step="0.01" placeholder="18.50" value={form.price} onChange={e=>setForm({...form,price:e.target.value})}/></div>
          </div>
          <div style={{ marginBottom:8 }}><div style={{ fontSize:11,color:C.soft,marginBottom:4 }}>Nazwa *</div><input {...inp} placeholder="Morele suszone 500g" value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:8 }}>
            <div><div style={{ fontSize:11,color:C.soft,marginBottom:4 }}>VAT %</div>
            ...
            <div><div style={{ fontSize:11,color:C.soft,marginBottom:4 }}>Stan mag.</div><input {...inp} type="number" placeholder="0" value={form.initial_quantity} onChange={e=>setForm({...form,initial_quantity:e.target.value})}/></div>
            <div><div style={{ fontSize:11,color:C.soft,marginBottom:4 }}>Waga (kg)</div><input {...inp} type="number" step="0.01" placeholder="0.5" value={form.weight_kg} onChange={e=>setForm({...form,weight_kg:e.target.value})}/></div>
          </div>
          <div style={{ marginBottom:12 }}><div style={{ fontSize:11,color:C.soft,marginBottom:4 }}>Marka</div><input {...inp} placeholder="Kaukaz" value={form.brand} onChange={e=>setForm({...form,brand:e.target.value})}/></div>
          ...
          <button onClick={save} disabled={saving} style={{ width:"100%",padding:"12px",borderRadius:9,border:"none",background:saving?C.soft:C.navy,color:"#fff",fontSize:14,fontWeight:600,cursor:saving?"not-allowed":"pointer",fontFamily:"inherit" }}>
  ...
  <div style={{ padding:"12px 12px 20px",display:"flex",flexDirection:"column",gap:10 }}>
    <Card style={{ padding:"16px" }}>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
        {[{label:"Wszystkich zamówień",value:stats.totalOrders,color:C.accent},{label:"Nowych zamówień",value:stats.newOrders,color:C.amber},{label:"Produktów",value:stats.totalProducts,color:C.green},{label:"Niski stan",value:stats.lowStock,color:C.red}].map(s=>(
    ...
    <Card style={{ padding:"16px" }}>
      <div style={{ fontSize:11,fontWeight:700,color:C.soft,letterSpacing:1,marginBottom:12 }}>ZALOGOWANY</div>
      <div style={{ display:"flex",alignItems:"center",gap:12 }}>
        {user?.user_metadata?.avatar_url&&<img src={user.user_metadata.avatar_url} style={{ width:40,height:40,borderRadius:"50%" }}/>}
        <div>
          <div style={{ fontSize:14,fontWeight:600,color:C.text }}>{user?.user_metadata?.full_name||"Użytkownik"}</div>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const {data:{subscription}}=supabase.auth.onAuthStateChange((_,session)=>{setSession(session);setLoading(false);});
  ...
  return (
    <div style={{ fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif",background:C.bg,minHeight:"100vh",maxWidth:480,margin:"0 auto" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap');*{box-sizing:border-box;margin:0;padding:0;}::-webkit-scrollbar{width:0;height:0;}button:active{opacity:0.85;}`}</style>
      <TopBar tab={tab} setTab={setTab} stats={stats} user={session.user} onLogout={logout}/>
      <StatRow stats={stats}/>
      {tab==="orders"&&<OrdersTab/>}
      {tab==="products"&&<ProductsTab/>}
      {tab==="channels"&&<ChannelsTab/>}
      {tab==="analytics"&&<AnalyticsTab stats={stats} user={session.user}/>}
    </div>
  );
}
