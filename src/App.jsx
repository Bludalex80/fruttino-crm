...
const Modal = ({ title, onClose, children, width=480 }) => (
  <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}>
    <div style={{ background:C.surface,borderRadius:16,width:"100%",maxWidth:width,boxShadow:"0 24px 80px rgba(0,0,0,0.3)",maxHeight:"90vh",overflow:"auto" }}>
...
// ── PROFILE MENU ───────────────────────────────────────────
const ProfileMenu = ({ user, onLogout, theme, setTheme }) => {
  ...
  return (
    <div ref={ref} style={{ position:"relative" }}>
      <button onClick={()=>setOpen(!open)} style={{ width:34,height:34,borderRadius:"50%",overflow:"hidden",border:"2px solid rgba(255,255,255,0.2)",cursor:"pointer",background:"rgba(255,255,255,0.15)",display:"flex",alignItems:"center",justifyContent:"center",padding:0 }}>
        {user?.user_metadata?.avatar_url
          ? <img src={user.user_metadata.avatar_url} style={{ width:34,height:34,objectFit:"cover" }}/>
          : <span style={{ fontSize:12,fontWeight:700,color:"#fff" }}>{user?.email?.slice(0,2).toUpperCase()}</span>
        }
      </button>
      {open && (
        <div style={{ position:"absolute",right:0,top:42,background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,boxShadow:"0 8px 32px rgba(0,0,0,0.15)",minWidth:220,zIndex:200 }}>
          ...
          <div style={{ padding:"10px 16px",borderTop:`1px solid ${C.border}` }}>
            ...
            <div style={{ display:"flex",gap:4 }}>
              {[{id:"light",icon:"☀️",label:"Jasny"},{id:"dark",icon:"🌙",label:"Ciemny"},{id:"auto",icon:"⚙️",label:"Auto"}].map(t=>(
                <button key={t.id} onClick={()=>setTheme(t.id)} style={{ flex:1,padding:"6px 2px",borderRadius:7,border:`1px solid ${theme===t.id?C.accent:C.border}`,background:theme===t.id?C.blueBg:C.surface,color:theme===t.id?C.accent:C.mid,fontSize:11,cursor:"pointer",fontFamily:"inherit",fontWeight:theme===t.id?600:400 }}>{t.icon} {t.label}</button>
              ))}
            </div>
          </div>
          <button onClick={onLogout} style={{ width:"100%",padding:"10px 16px",border:"none",borderTop:`1px solid ${C.border}`,background:"transparent",cursor:"pointer",display:"flex",alignItems:"center",gap:10,fontSize:13,color:C.red,fontFamily:"inherit",textAlign:"left",borderRadius:"0 0 12px 12px" }}>
...
const TopBar = ({ user, onLogout, theme, setTheme }) => {
  ...
  return (
    <header style={{ height:54,background:C.surface,borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",paddingLeft:236,paddingRight:20,gap:12,position:"fixed",top:0,left:0,right:0,zIndex:90,boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
      <button style={{ display:"flex",alignItems:"center",gap:6,padding:"6px 12px",borderRadius:8,border:`1px solid ${C.border}`,background:C.alt,fontSize:13,color:C.mid,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap",flexShrink:0 }}>
        ⚡ Szybki dostęp <span style={{ fontSize:10 }}>▾</span>
      </button>
      <div style={{ flex:1,maxWidth:460,position:"relative" }}>
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Szukaj zamówień, produktów, klientów..." style={{ width:"100%",padding:"7px 12px 7px 32px",borderRadius:8,border:`1px solid ${C.border}`,fontSize:13,fontFamily:"inherit",background:C.alt,outline:"none",color:C.text,boxSizing:"border-box" }}/>
      </div>
      <div style={{ display:"flex",alignItems:"center",gap:6,marginLeft:"auto" }}>
        ...
        <button style={{ width:34,height:34,borderRadius:8,border:`1px solid ${C.border}`,background:C.surface,cursor:"pointer",fontSize:14,color:C.mid,display:"flex",alignItems:"center",justifyContent:"center",position:"relative" }}>
          🔔<span style={{ position:"absolute",top:-4,right:-4,background:C.red,color:"#fff",borderRadius:"50%",fontSize:9,fontWeight:700,width:15,height:15,display:"flex",alignItems:"center",justifyContent:"center" }}>3</span>
        </button>
        ...
        <ProfileMenu user={user} onLogout={onLogout} theme={theme} setTheme={setTheme}/>
      </div>
    </header>
  );
};
...
// ── SIDEBAR ────────────────────────────────────────────────
const Sidebar = ({ tab, setTab, subTab, setSubTab, stats }) => {
  ...
  const nav = [
    ...
    { id:"orders", icon:"🛒", label:"Zamówienia", badge:stats.newOrders,
      sub:[
        {id:"orders-list",label:"Lista zamówień"},
        {id:"orders-invoices",label:"Faktury"},
        {id:"orders-returns",label:"Zwroty"},
        {id:"orders-clients",label:"Klienci"},
        {id:"orders-statuses",label:"Statusy zamówień"},
        {id:"orders-templates",label:"Szablony E-mail/SMS"},
        {id:"orders-actions",label:"Automatyczne akcje"},
        {id:"orders-exports",label:"Wydruki i eksporty"},
        {id:"orders-imports",label:"Import przelewów"},
        {id:"orders-settings",label:"Ustawienia"},
      ]
    },
    { id:"products", icon:"🌿", label:"Produkty", badge:stats.lowStock>0?stats.lowStock:null,
      sub:[
        {id:"products-list",label:"Lista produktów"},
        {id:"products-stock",label:"Kontrola magazynu"},
        {id:"products-actions",label:"Automatyczne akcje"},
        {id:"products-import",label:"Import/Eksport"},
        {id:"products-settings",label:"Ustawienia"},
      ]
    },
  ...
  return (
    <aside style={{ width:220,background:C.navy,minHeight:"100vh",display:"flex",flexDirection:"column",position:"fixed",left:0,top:0,bottom:0,zIndex:100,overflowY:"auto" }}>
      <div style={{ padding:"16px",borderBottom:"1px solid rgba(255,255,255,0.08)",flexShrink:0 }}>
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
          ...
          <div>
            <div style={{ fontSize:14,fontWeight:700,color:"#fff",lineHeight:1 }}>Fruttino</div>
            <div style={{ fontSize:9,color:"rgba(255,255,255,0.4)",letterSpacing:1.5,marginTop:2 }}>CRM PANEL</div>
          </div>
        </div>
      </div>
      <nav style={{ flex:1,padding:"8px 6px" }}>
        {nav.map(item=>{
          ...
          const hasActiveSub=item.sub&&item.sub.some(s=>s.id===subTab);
            ...
            <div key={item.id}>
              ...
              {item.sub&&isExp&&(
                <div style={{ marginLeft:10,marginBottom:4 }}>
                  {item.sub.map(s=>(
                    <button key={s.id} onClick={()=>{setTab(item.id);setSubTab(s.id);}} style={{ width:"100%",padding:"7px 10px",border:"none",borderLeft:`2px solid ${subTab===s.id?"#60a5fa":"rgba(255,255,255,0.08)"}`,background:"transparent",color:subTab===s.id?"#fff":"rgba(255,255,255,0.4)",fontSize:12,cursor:"pointer",fontFamily:"inherit",textAlign:"left",marginBottom:1,borderRadius:"0 6px 6px 0",display:"block" }}>
                      {s.label}
                    </button>
                  ))}
                </div>
      ...
      <div style={{ padding:"10px 14px",borderTop:"1px solid rgba(255,255,255,0.08)",flexShrink:0 }}>
        ...
        {[{dot:C.orange,label:"Allegro"},{dot:"#4ade80",label:"WooCommerce"}].map(c=>(
          <div key={c.label} style={{ display:"flex",alignItems:"center",gap:5,fontSize:11,color:"rgba(255,255,255,0.4)",marginBottom:3 }}>
            <span style={{ width:5,height:5,borderRadius:"50%",background:c.dot,display:"block",flexShrink:0 }}/>{c.label}
          </div>
        ))}
      </div>
    </aside>
...
// ── MOBILE TOP BAR ─────────────────────────────────────────
const MobileTopBar = ({ tab, setTab, stats, user, onLogout, theme, setTheme }) => {
  const tabs=[
    ...
    <div style={{ background:C.navy,position:"sticky",top:0,zIndex:100 }}>
      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px 6px" }}>
        <div style={{ display:"flex",alignItems:"center",gap:8 }}>
          ...
          <span style={{ fontSize:14,fontWeight:700,color:"#fff" }}>Fruttino CRM</span>
        </div>
        <ProfileMenu user={user} onLogout={onLogout} theme={theme} setTheme={setTheme}/>
      </div>
...
const StatCards = ({ stats, isMobile }) => (
  <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:isMobile?8:16,marginBottom:isMobile?0:24 }}>
    {[
      {label:"Nowe zamówienia",value:stats.newOrders,color:C.amber,icon:"🔔"},
      {label:"Wszystkie zamów.",value:stats.totalOrders,color:C.accent,icon:"📦"},
      {label:"Produkty",value:stats.totalProducts,color:C.green,icon:"🌿"},
      {label:"Niski stan",value:stats.lowStock,color:C.red,icon:"⚠️"},
    ].map(s=>(
  ...
  <div>
    ...
    <div style={{ display:"grid",gridTemplateColumns:"2fr 1fr",gap:20 }}>
      <Card style={{ padding:20 }}>
        ...
        {[
          {icon:"📦",text:"Oczekiwanie na pierwsze zamówienie z Allegro",time:"Skonfiguruj integrację",color:C.orange},
          {icon:"🌿",text:"Dodano produkt: Morele suszone 500g",time:"Dziś",color:C.green},
          {icon:"🔗",text:"6 kanałów sprzedaży skonfigurowanych",time:"Gotowe",color:C.blue},
          {icon:"✅",text:"CRM Panel uruchomiony pomyślnie",time:"Dziś",color:C.green},
        ].map((a,i)=>(
...
// ── ORDERS TAB ─────────────────────────────────────────────
const OrdersTab = ({ subTab, isMobile }) => {
  ...
  const subLabels={
    "orders-invoices":"Faktury","orders-returns":"Zwroty","orders-clients":"Klienci",
    "orders-statuses":"Statusy zamówień","orders-templates":"Szablony E-mail/SMS",
    "orders-actions":"Automatyczne akcje","orders-exports":"Wydruki i eksporty",
    "orders-imports":"Import przelewów","orders-settings":"Ustawienia",
  };

  if(subTab&&subTab!=="orders-list") return <ComingSoon title={subLabels[subTab]||subTab}/>;
  ...
  if(isMobile) return (
    <div style={{ padding:"12px 12px 20px" }}>
      <div style={{ display:"flex",gap:8,marginBottom:12,flexWrap:"wrap" }}>
        ...
        <select {...sel} value={fStatus} onChange={e=>setFStatus(e.target.value)}><option value="all">Każdy status</option>{Object.entries(STATUS).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}</select>
      ...
      {loading?<Spinner/>:filtered.length===0?<Empty text="Brak zamówień"/>:(
        <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
          {filtered.map(o=>{
            const st=STATUS[o.status]||STATUS.new;const src=SOURCE[o.channels?.type]||SOURCE.allegro;const isOpen=selected===o.id;
            return (
              <Card key={o.id} style={{ overflow:"hidden",border:isOpen?`1px solid ${C.accent}`:`1px solid ${C.border}` }}>
                <div onClick={()=>setSelected(isOpen?null:o.id)} style={{ padding:"13px 14px",cursor:"pointer" }}>
                  ...
                  <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
                    ...
                    <div style={{ textAlign:"right",flexShrink:0,marginLeft:12 }}><div style={{ fontSize:19,fontWeight:800,color:C.navy }}>{parseFloat(o.total_amount||0).toFixed(2)}</div><div style={{ fontSize:10,color:C.soft }}>{o.currency||"PLN"}</div></div>
                  </div>
      ...
      <Card style={{ overflow:"hidden" }}>
        <table style={{ width:"100%",borderCollapse:"collapse" }}>
          ...
          <tbody>
            ...
            :filtered.length===0?<tr><td colSpan={6}><Empty text="Brak zamówień. Gdy klient złoży zamówienie — pojawi się tutaj."/></td></tr>
            :filtered.map((o,i)=>{
              const st=STATUS[o.status]||STATUS.new;const src=SOURCE[o.channels?.type]||SOURCE.allegro;const isOpen=selected===o.id;
              return [
                <tr key={o.id} style={{ borderBottom:`1px solid ${C.borderLight}`,background:isOpen?`${C.accent}08`:i%2===0?C.surface:"#fafafa" }}>
                  ...
                  <td style={{ padding:"14px 16px" }}><div style={{ fontSize:15,fontWeight:700,color:C.navy,fontFamily:"monospace" }}>{parseFloat(o.total_amount||0).toFixed(2)}</div><div style={{ fontSize:11,color:C.soft }}>{o.currency||"PLN"}</div></td>
                ...
                isOpen&&<tr key={`${o.id}-d`}><td colSpan={6} style={{ padding:"0 16px 16px",background:`${C.accent}06`,borderBottom:`1px solid ${C.border}` }}><div style={{ padding:"16px",background:C.surface,borderRadius:10,border:`1px solid ${C.border}`,marginTop:8 }}><div style={{ fontSize:13,color:C.soft,marginBottom:12 }}>Zmień status:</div><div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>{Object.entries(STATUS).map(([k,v])=><button key={k} onClick={()=>updateStatus(o.id,k)} disabled={o.status===k||updating===o.id} style={{ padding:"8px 16px",borderRadius:8,fontSize:13,fontWeight:500,cursor:o.status===k?"default":"pointer",border:`1px solid ${o.status===k?v.dot:C.border}`,background:o.status===k?v.bg:C.surface,color:o.status===k?v.text:C.mid,fontFamily:"inherit",opacity:updating===o.id?0.6:1 }}>{v.label}</button>)}</div></div></td></tr>
              ];
            })}
          </tbody>
        </table>
...
// ── PRODUCTS TAB ───────────────────────────────────────────
const ProductsTab = ({ subTab, isMobile }) => {
  ...
  const downloadTemplate=()=>{
    const lines=[
      "# FRUTTINO CRM — Master plik importu produktów",
      "# * = pole obowiązkowe | bez * = pole opcjonalne",
      "sku*,name*,price*,vat_rate,initial_quantity,brand,weight_kg,description,ean,category,skład,kraj_pochodzenia,alergeny,stan,dostawa_dni",
      "MOR-500,Morele suszone 500g,18.50,5,100,Kaukaz,0.5,Premium suszone morele,5901234567890,Owoce suszone,morele,Turcja,orzechy,nowy,2",
    ];
  ...
  if(subTab==="products-import") return (
    <div>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20 }}>
        <h2 style={{ fontSize:20,fontWeight:700,color:C.text }}>Import / Eksport katalogu</h2>
      </div>
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:20 }}>
        <Card style={{ padding:24 }}>
          ...
          <button onClick={downloadTemplate} style={{ width:"100%",padding:10,borderRadius:8,border:`1px solid ${C.accent}`,background:C.blueBg,color:C.accent,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit" }}>
            📋 Pobierz plik wzorcowy (master template)
          </button>
        </Card>
        <Card style={{ padding:24 }}>
          <div style={{ fontSize:32,marginBottom:12 }}>📤</div>
          <h3 style={{ fontSize:16,fontWeight:700,color:C.text,marginBottom:8 }}>Eksport katalogu</h3>
          <p style={{ fontSize:13,color:C.soft,marginBottom:16,lineHeight:1.6 }}>Pobierz pełny katalog produktów w formacie CSV. Możesz go edytować i zaimportować z powrotem.</p>
          <div style={{ background:C.alt,borderRadius:10,padding:16,marginBottom:16 }}>
            {["SKU, nazwa, cena, VAT","Stan magazynowy","Marka, waga, opis","EAN, kategoria","Parametry (skład, alergeny, kraj)"].map(item=>(
              <div key={item} style={{ fontSize:12,color:C.mid,marginBottom:4,display:"flex",alignItems:"center",gap:6 }}><span style={{ color:C.green }}>✓</span>{item}</div>
            ))}
          </div>
          <button onClick={exportCSV} style={{ width:"100%",padding:10,borderRadius:8,border:"none",background:C.navy,color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit" }}>
            ⬇ Eksportuj katalog ({products.length} produktów)
          </button>
        </Card>
      </div>
      <Card style={{ padding:24 }}>
        ...
        <table style={{ width:"100%",borderCollapse:"collapse" }}>
          ...
          <tbody>
            {[
              ["sku","tekst","⭐ TAK","Unikalny kod produktu. Np: MOR-500"],
              ["name","tekst","⭐ TAK","Pełna nazwa produktu. Np: Morele suszone 500g"],
              ["price","liczba","⭐ TAK","Cena w PLN. Np: 18.50"],
              ["vat_rate","liczba","○ nie","Stawka VAT: 23, 8, 5, 0. Domyślnie: 23"],
              ["initial_quantity","liczba","○ nie","Stan magazynowy. Domyślnie: 0"],
              ["brand","tekst","○ nie","Marka / producent. Np: Kaukaz"],
              ["weight_kg","liczba","○ nie","Waga w kg. Np: 0.5"],
              ["description","tekst","○ nie","Opis produktu do oferty"],
              ["ean","tekst","○ nie","Kod EAN/GTIN. Np: 5901234567890"],
              ["category","tekst","○ nie","Kategoria. Np: Owoce suszone"],
              ["skład","tekst","○ nie","Skład produktu. Np: morele 100%"],
              ["kraj_pochodzenia","tekst","○ nie","Np: Turcja, Iran, Uzbekistan"],
              ["alergeny","tekst","○ nie","Np: może zawierać orzechy"],
              ["stan","tekst","○ nie","nowy / używany. Domyślnie: nowy"],
              ["dostawa_dni","liczba","○ nie","Czas wysyłki w dniach. Np: 2"],
            ].map(([field,type,req,desc],i)=>(
              <tr key={field} style={{ borderBottom:`1px solid ${C.borderLight}`,background:i%2===0?C.surface:"#fafafa" }}>
  ...
  if(subTab&&!["products-list","products-import"].includes(subTab)) return <ComingSoon title={subTab}/>;
  ...
  const Form=()=>(
    <Card style={{ padding:isMobile?16:24,marginBottom:isMobile?12:20,borderLeft:`4px solid ${C.accent}` }}>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:12,marginBottom:12 }}>
        <div><div style={{ fontSize:11,color:C.soft,marginBottom:4,fontWeight:600 }}>VAT %</div><select {...inp} value={form.vat_rate} onChange={e=>setForm({...form,vat_rate:e.target.value})}><option value="23">23%</option><option value="8">8%</option><option value="5">5%</option><option value="0">0%</option></select></div>
        ...
        <div><div style={{ fontSize:11,color:C.soft,marginBottom:4,fontWeight:600 }}>Marka</div><input {...inp} placeholder="Kaukaz" value={form.brand} onChange={e=>setForm({...form,brand:e.target.value})}/></div>
      </div>
      ...
      <button onClick={save} disabled={saving} style={{ padding:"11px 24px",borderRadius:9,border:"none",background:saving?C.soft:C.navy,color:"#fff",fontSize:14,fontWeight:600,cursor:saving?"not-allowed":"pointer",fontFamily:"inherit" }}>{saving?"Zapisywanie...":"💾 Zapisz"}</button>
    </Card>
  );
  ...
  if(isMobile) return (
    <div style={{ padding:"12px 12px 20px" }}>
      <div style={{ display:"flex",gap:8,marginBottom:12 }}>
        ...
        <button onClick={()=>{setShowForm(!showForm);setFormError("");}} style={{ padding:"9px 14px",borderRadius:8,border:"none",background:C.navy,color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap" }}>{showForm?"✕":"+Dodaj"}</button>
      </div>
      {showForm&&<Form/>}
      {error&&<div style={{ fontSize:12,color:C.red,marginBottom:10,padding:"8px 12px",background:C.redBg,borderRadius:8 }}>⚠ {error}</div>}
      {loading?<Spinner/>:products.length===0?<Empty text="Brak produktów!"/>:(
        <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
          {products.map(p=>{const inv=p.inventory?.[0]||{};const stock=(inv.quantity||0)-(inv.reserved||0);const isLow=inv.min_threshold!==null&&stock<=(inv.min_threshold||5);
            return <Card key={p.id} style={{ padding:"13px 14px",borderLeft:`4px solid ${isLow?C.red:C.accent}` }}><div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}><div style={{ flex:1 }}><div style={{ display:"flex",alignItems:"center",gap:6,marginBottom:5,flexWrap:"wrap" }}><span style={{ fontSize:14,fontWeight:700,color:C.text }}>{p.name}</span>{p.status==="draft"&&<span style={{ fontSize:10,background:C.amberBg,color:"#92400e",padding:"2px 7px",borderRadius:100,fontWeight:600 }}>SZKIC</span>}</div><span style={{ fontSize:11,background:C.alt,color:C.mid,padding:"2px 8px",borderRadius:6,border:`1px solid ${C.border}` }}>{p.sku}</span></div><div style={{ textAlign:"right",flexShrink:0,marginLeft:10 }}><div style={{ fontSize:17,fontWeight:800,color:C.navy }}>{parseFloat(p.price||0).toFixed(2)} zł</div><div style={{ fontSize:13,fontWeight:700,marginTop:3,color:isLow?C.red:C.green }}>{stock} szt.</div></div></div></Card>;
          })}
        </div>
      )}
    </div>
    ...
    <div>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20 }}>
        ...
        <div style={{ display:"flex",gap:10 }}>
          ...
          <button onClick={()=>{setShowForm(!showForm);setFormError("");}} style={{ padding:"8px 20px",borderRadius:8,border:"none",background:showForm?C.soft:C.navy,color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap" }}>{showForm?"✕ Anuluj":"+ Dodaj produkt"}</button>
        </div>
      </div>
      {showForm&&<Form/>}
      {error&&<div style={{ fontSize:13,color:C.red,marginBottom:16,padding:"10px 16px",background:C.redBg,borderRadius:8 }}>⚠ {error}</div>}
      <Card style={{ overflow:"hidden" }}>
        <table style={{ width:"100%",borderCollapse:"collapse" }}>
          <thead><tr style={{ background:C.alt,borderBottom:`1px solid ${C.border}` }}>{["SKU","Produkt","VAT","Cena","Stan magazynowy","Status",""].map(h=><th key={h} style={{ padding:"12px 16px",textAlign:"left",fontSize:11,color:C.soft,fontWeight:600,letterSpacing:0.8 }}>{h.toUpperCase()}</th>)}</tr></thead>
          <tbody>
            {loading?<tr><td colSpan={7}><Spinner/></td></tr>
            :products.length===0?<tr><td colSpan={7}><Empty text="Brak produktów. Dodaj pierwszy!"/></td></tr>
            :products.map((p,i)=>{const inv=p.inventory?.[0]||{};const stock=(inv.quantity||0)-(inv.reserved||0);const isLow=inv.min_threshold!==null&&stock<=(inv.min_threshold||5);
              return <tr key={p.id} style={{ borderBottom:`1px solid ${C.borderLight}`,background:i%2===0?C.surface:"#fafafa" }}>
                <td style={{ padding:"14px 16px",fontFamily:"monospace",fontSize:12,color:C.soft }}>{p.sku}</td>
                <td style={{ padding:"14px 16px" }}><div style={{ fontSize:14,fontWeight:600,color:C.text,marginBottom:2 }}>{p.name}</div>{p.brand&&<div style={{ fontSize:12,color:C.soft }}>{p.brand}</div>}</td>
                ...
                <td style={{ padding:"14px 16px" }}><div style={{ display:"flex",alignItems:"center",gap:10 }}><div style={{ flex:1,maxWidth:100,height:6,background:C.border,borderRadius:3,overflow:"hidden" }}><div style={{ height:"100%",width:`${Math.min(((inv.quantity||0)/100)*100,100)}%`,background:isLow?C.red:C.accent,borderRadius:3 }}/></div><span style={{ fontSize:13,fontWeight:700,color:isLow?C.red:C.green }}>{stock} szt.</span></div></td>
                ...
                <td style={{ padding:"14px 16px" }}><button style={{ padding:"6px 14px",borderRadius:7,border:`1px solid ${C.border}`,background:C.surface,fontSize:12,cursor:"pointer",color:C.mid,fontFamily:"inherit" }}>Edytuj</button></td>
              </tr>;
            })}
          </tbody>
        </table>
...
const ChannelsTab = ({ isMobile }) => {
  ...
  const testConn=async(id)=>{ setTesting(id);const res=await apiFetch(`/channels/${id}/test`,{method:"POST",body:"{}"});setTestResult(prev=>({...prev,[id]:res.data||res}));setTesting(null); };
  const openConfig=(ch)=>{ setConfigModal(ch);setConfigData({}); };
  const saveConfig=async()=>{
    setSaving(true);
    await apiFetch(`/channels/${configModal.id}`,{method:"PUT",body:JSON.stringify(configData)});
    setSaving(false);setConfigModal(null);
    const res=await apiFetch("/channels");if(res.success) setChannels(res.data||[]);
  };
    ...
    <div style={{ padding:isMobile?"12px 12px 20px":0 }}>
      ...
      {loading?<Spinner/>:(
        <div style={{ display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(auto-fill,minmax(320px,1fr))",gap:isMobile?8:16 }}>
          {channels.map(ch=>{
            const src=ch.type==="allegro"?SOURCE.allegro:SOURCE.woocommerce;const tr=testResult[ch.id];
            return (
              <Card key={ch.id} style={{ padding:20 }}>
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12 }}>
                  <div><div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:6 }}><Pill label={ch.type==="allegro"?"Allegro":"WooCommerce"} color={src.color} bg={src.bg} textColor={src.color}/><span style={{ fontSize:12,background:C.alt,color:C.mid,padding:"2px 8px",borderRadius:6,border:`1px solid ${C.border}` }}>{ch.country}</span></div><div style={{ fontSize:16,fontWeight:700,color:C.text }}>{ch.name}</div>{ch.shop_url&&<div style={{ fontSize:12,color:C.soft,marginTop:2 }}>{ch.shop_url}</div>}</div>
                  <div style={{ display:"flex",alignItems:"center",gap:6 }}><span style={{ width:8,height:8,borderRadius:"50%",background:ch.is_active?C.green:C.soft,display:"block" }}/><span style={{ fontSize:12,color:ch.is_active?C.green:C.soft }}>{ch.is_active?"Aktywny":"Nieaktywny"}</span></div>
                </div>
                ...
                <div style={{ display:"flex",gap:8 }}>
                  ...
                  <button onClick={()=>openConfig(ch)} style={{ padding:"8px 16px",borderRadius:8,border:"none",background:C.accent,color:"#fff",fontSize:13,cursor:"pointer",fontFamily:"inherit",fontWeight:600 }}>⚙ Konfiguruj</button>
                </div>
              </Card>
            );
          })}
      ...
      {configModal&&(
        <Modal title={`Konfiguracja: ${configModal.name}`} onClose={()=>setConfigModal(null)}>
          ...
          <div style={{ display:"flex",gap:10,marginTop:20 }}>
            ...
            <button onClick={saveConfig} disabled={saving} style={{ flex:2,padding:10,borderRadius:8,border:"none",background:C.navy,color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",opacity:saving?0.7:1 }}>{saving?"Zapisywanie...":"💾 Zapisz i połącz"}</button>
          </div>
        </Modal>
      )}
    </div>
...
const CouriersTab = ({ isMobile }) => {
  ...
  const couriers=[
    { id:"allegro_delivery",name:"Wysyłam z Allegro",logo:"🛒",color:C.orange,bg:C.orangeBg,
      description:"Oficjalna usługa dostawy Allegro. Automatyczne nadawanie przesyłek z panelu.",
      features:["Automatyczne etykiety","Śledzenie przesyłek","Allegro Smart!","InPost, DPD, DHL i inne"],
      fields:[{key:"allegro_account",label:"Konto Allegro",ph:"login@allegro.pl"},{key:"allegro_delivery_token",label:"Token API dostawy",ph:"Token z ustawień konta Allegro"}]
    },
    { id:"inpost",name:"InPost Paczkomaty",logo:"📦",color:"#f59e0b",bg:"#fffde7",
      description:"Sieć Paczkomatów InPost — najpopularniejsza dostawa w Polsce.",
      features:["24/7 odbiór","Paczkomaty","Kurier InPost","API v3"],
      fields:[{key:"inpost_token",label:"Token API InPost",ph:"Token z manager.inpost.pl"},{key:"inpost_org_id",label:"ID organizacji",ph:"Np: 12345"}]
    },
    { id:"dpd",name:"DPD Polska",logo:"🚚",color:"#dc2626",bg:"#fff0f0",
      description:"Kurier DPD — szybka dostawa na terenie Polski i Europy.",
      features:["Dostawa krajowa","Dostawa do Europy","Kurier na żądanie","Śledzenie online"],
      fields:[{key:"dpd_login",label:"Login DPD",ph:"login@firma.pl"},{key:"dpd_password",label:"Hasło API",ph:"Hasło z umowy DPD"},{key:"dpd_fid",label:"FID (numer umowy)",ph:"Np: 123456"}]
    },
    { id:"dhl",name:"DHL Express",logo:"✈️",color:"#d97706",bg:"#fffbeb",
      description:"DHL Express — ekspresowa dostawa krajowa i międzynarodowa.",
      features:["Ekspresowa dostawa","Dostawa międzynarodowa","Śledzenie real-time","Ubezpieczenie"],
      fields:[{key:"dhl_account",label:"Numer konta DHL",ph:"Np: 123456789"},{key:"dhl_key",label:"Klucz API",ph:"Klucz z portalu DHL"},{key:"dhl_password",label:"Hasło API",ph:"Hasło API DHL"}]
    },
  ];
  ...
  const saveConfig=async()=>{
    ...
    setConnected(prev=>({...prev,[configModal.id]:true}));
    setSaving(false);setConfigModal(null);
  };
  ...
  return (
    <div style={{ padding:isMobile?"12px 12px 20px":0 }}>
      {!isMobile&&<div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20 }}>
        <h2 style={{ fontSize:20,fontWeight:700,color:C.text }}>Kurierzy</h2>
        <div style={{ fontSize:13,color:C.soft }}>{Object.values(connected).filter(Boolean).length} połączonych</div>
      </div>}
      <div style={{ display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(auto-fill,minmax(320px,1fr))",gap:isMobile?12:16 }}>
        {couriers.map(c=>{
          ...
          return (
            <Card key={c.id} style={{ padding:20,border:isConn?`1px solid ${C.green}`:`1px solid ${C.border}` }}>
              ...
              <div style={{ display:"flex",flexWrap:"wrap",gap:5,marginBottom:14 }}>
                {c.features.map(f=><span key={f} style={{ fontSize:11,background:C.alt,color:C.mid,padding:"3px 8px",borderRadius:100,border:`1px solid ${C.border}` }}>{f}</span>)}
              </div>
              <button onClick={()=>{setConfigModal(c);setConfigData({});}} style={{ width:"100%",padding:"9px",borderRadius:8,border:isConn?`1px solid ${C.green}`:"none",background:isConn?C.greenBg:C.navy,color:isConn?"#065f46":"#fff",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit" }}>
                {isConn?"✅ Skonfigurowany — edytuj":"⚙ Konfiguruj połączenie API"}
              </button>
            </Card>
          );
        })}
      ...
      {configModal&&(
        <Modal title={`Konfiguracja: ${configModal.name}`} onClose={()=>setConfigModal(null)}>
          <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:20,padding:"12px 14px",background:configModal.bg,borderRadius:10,border:`1px solid ${configModal.color}33` }}>
            <span style={{ fontSize:24 }}>{configModal.logo}</span>
            <div><div style={{ fontSize:14,fontWeight:600,color:C.text }}>{configModal.name}</div><div style={{ fontSize:12,color:C.soft }}>{configModal.description}</div></div>
          </div>
          {configModal.fields.map(f=>(
            <div key={f.key} style={{ marginBottom:14 }}>
              <div style={{ fontSize:11,fontWeight:600,color:C.soft,marginBottom:6 }}>{f.label}</div>
              <input {...inp} placeholder={f.ph} value={configData[f.key]||""} onChange={e=>setConfigData({...configData,[f.key]:e.target.value})} type={f.key.includes("password")||f.key.includes("token")||f.key.includes("secret")?"password":"text"}/>
            </div>
          ))}
          <div style={{ padding:"10px 12px",background:C.alt,borderRadius:8,marginBottom:16 }}>
            <div style={{ fontSize:11,color:C.soft,marginBottom:4,fontWeight:600 }}>DOSTĘPNE FUNKCJE:</div>
            {configModal.features.map(f=><div key={f} style={{ fontSize:12,color:C.mid,display:"flex",alignItems:"center",gap:6,marginBottom:2 }}><span style={{ color:C.green }}>✓</span>{f}</div>)}
          </div>
          <div style={{ display:"flex",gap:10 }}>
            <button onClick={()=>setConfigModal(null)} style={{ flex:1,padding:10,borderRadius:8,border:`1px solid ${C.border}`,background:C.surface,fontSize:13,cursor:"pointer",color:C.mid,fontFamily:"inherit" }}>Anuluj</button>
            <button onClick={saveConfig} disabled={saving} style={{ flex:2,padding:10,borderRadius:8,border:"none",background:C.navy,color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",opacity:saving?0.7:1 }}>{saving?"Łączenie...":"💾 Zapisz i połącz"}</button>
          </div>
        </Modal>
      )}
    </div>
    ...
    <Card style={{ padding:isMobile?14:20 }}>
      ...
      <div style={{ display:"flex",alignItems:"center",gap:14 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:11,color:C.soft,marginTop:2 }}>Tenant: Bazár Kaukazu</div>
        </div>
      </div>
    </Card>
  </div>
...
export default function App() {
  ...
  const renderTab=(mobile=false)=>{
    if(tab==="dashboard")  return <DashboardTab stats={stats}/>;
    if(tab==="orders")     return <OrdersTab subTab={subTab||"orders-list"} isMobile={mobile}/>;
    if(tab==="products")   return <ProductsTab subTab={subTab||"products-list"} isMobile={mobile}/>;
    if(tab==="channels")   return <ChannelsTab isMobile={mobile}/>;
    if(tab==="couriers")   return <CouriersTab isMobile={mobile}/>;
    if(tab==="analytics")  return <AnalyticsTab stats={stats} user={session.user} isMobile={mobile}/>;
    return null;
  };
    ...
    <div style={{ fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif",background:C.bg,minHeight:"100vh" }}>
      ...
      {isMobile ? (
        <div style={{ maxWidth:480,margin:"0 auto" }}>
          <MobileTopBar tab={tab} setTab={setTab} stats={stats} user={session.user} onLogout={logout} theme={theme} setTheme={setTheme}/>
          <div style={{ padding:"12px 0 0" }}>
            <div style={{ padding:"0 12px",marginBottom:12 }}><StatCards stats={stats} isMobile={true}/></div>
            {renderTab(true)}
          </div>
        </div>
      ) : (
        <>
          <Sidebar tab={tab} setTab={setTab} subTab={subTab} setSubTab={setSubTab} stats={stats}/>
          <TopBar user={session.user} onLogout={logout} theme={theme} setTheme={setTheme}/>
